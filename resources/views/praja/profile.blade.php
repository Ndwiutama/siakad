...
<div class="max-w-5xl mx-auto mt-8 flex flex-col gap-6" x-data="{
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl shadow p-6 flex flex-col gap-2 border col-span-2">
            ...
            <div class="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
                <div>Nama lengkap</div><div>: <span x-text="data.nama"></span></div>
                <div>Jenis kelamin</div><div>: <span x-text="data.jk"></span></div>
                <div>Tempat, Tanggal Lahir</div><div>: <span x-text="data.ttl"></span></div>
                <div>Agama</div><div>: <span x-text="data.agama"></span></div>
                <div>Asdaf</div><div>: <span x-text="data.asdaf"></span></div>
                <div>Wisma</div><div>: <span x-text="data.wisma"></span></div>
                <div>Nomor Induk Kependudukan (NIK)</div><div>: <span x-text="data.nik"></span></div>
                <div>Nomor Kartu Keluarga</div><div>: <span x-text="data.kk"></span></div>
                <div>Kewarganegaraan</div><div>: <span x-text="data.kewarganegaraan"></span></div>
                <div>Asal Sekolah</div><div>: <span x-text="data.sekolah"></span></div>
                <div>Nomor Handphone</div><div>: <span x-text="data.hp"></span></div>
                <div>Email</div><div>: <span x-text="data.email"></span></div>
            </div>
        </div>
        <!-- Data Akademik -->
        <div class="bg-white rounded-xl shadow p-6 flex flex-col gap-2 border col-span-1">
    ...
    <div x-show="modal" style="background: rgba(0,0,0,0.2)" class="fixed inset-0 z-50 flex items-center justify-center" x-transition>
        <div class="bg-white rounded-xl shadow-lg p-8 w-full max-w-lg relative">
            <button @click="modal = false" class="absolute top-2 right-2 text-gray-400 hover:text-red-500"><i class="fa fa-times text-xl"></i></button>
            ...
            <div class="flex justify-end gap-2 mt-6">
                ...
                <button @click="saveEdit()" class="px-4 py-2 rounded bg-blue-700 text-white hover:bg-blue-800">Simpan</button>
            </div>
        </div>
    </div>
</div>
